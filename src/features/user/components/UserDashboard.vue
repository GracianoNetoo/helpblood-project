...
watch(
  ...
  ([donorId, token]) => {
    notificationsStore.setActiveRecipient(donorId ? String(donorId) : null);
    helpRequestsStore.refreshApprovedFromSupabase();
    if (!donorId || !token) return;
    helpRequestsStore.refreshRequestsForRequester(donorId, token);
  },
  { immediate: true }
);

watch(
  [currentDonor, appointments, campaigns, requests],
  () => {
    notificationsStore.syncNotifications({
      donor: currentDonor.value,
      appointments: appointments.value,
      campaigns: campaigns.value,
      helpRequests: requests.value
    });
  },
...
const handleOpenNotificationTab = (tabId) => {
  if (!tabId) return;
...
<template>
  <div class="h-screen bg-[#FDFDFD] flex overflow-hidden font-sans selection:bg-rose-100 selection:text-rose-900">
    ...
    <NotificationsPanel
      ...
      @open-tab="handleOpenNotificationTab"
    />
  </div>
</template>
