...
const updateDonationDraft = ({ donorId, value }) => {
  donationDrafts.value[donorId] = value;
  donationTouched.value[donorId] = true;
};

const clearDonationDraft = (donorId) => {
  donationDrafts.value[donorId] = '';
  donationTouched.value[donorId] = true;
};
    ...
    <main class="flex-1 flex flex-col h-full bg-[#FAFAFA] relative overflow-hidden ring-1 ring-gray-200/50">
      ...
      <div class="flex-1 overflow-y-auto p-4 md:p-8 lg:p-10 scroll-smooth custom-scrollbar">
        ...
        <AdminOverviewTab
          ...
          :latest-requests="latestRequests"
          :scheduled-appointments="scheduledAppointments"
          :latest-appointments="latestAppointments"
          :donors="donors"
          ...
          @toggle-donor-status="toggleDonorStatus"
        ...
        <AdminCampaignsTab
          ...
          :campaign-touched="campaignTouched"
          :today-i-s-o="todayISO"
          :is-date-in-past="isDateInPast"
          :is-campaign-invalid="isCampaignInvalid"
          :campaigns="campaigns"
          :should-show-campaign-error="shouldShowCampaignError"
        ...
        <AdminDonorsTab
          ...
          :active-donors="activeDonors"
          :suspended-donors-count="suspendedDonorsCount"
          :donors-with-history-count="donorsWithHistoryCount"
          :total-donated-across-donors="totalDonatedAcrossDonors"
          :donors="donors"
          :filtered-donors="filteredDonors"
          :donor-search="donorSearch"
          :donor-status-filter="donorStatusFilter"
          :donation-min="donationMin"
          :donation-max="donationMax"
          :donation-step="donationStep"
          :format-total-donated="formatTotalDonated"
          :get-donor-initials="getDonorInitials"
          :get-donor-status-label="getDonorStatusLabel"
          :get-donor-status-classes="getDonorStatusClasses"
          :format-donation-date="formatDonationDate"
          :get-donation-draft-value="getDonationDraftValue"
          :get-donation-error-for="getDonationErrorFor"
          :can-save-donation="canSaveDonation"
          :format-liters="formatLiters"
          @update:donor-search="donorSearch = $event"
          @update:donor-status-filter="donorStatusFilter = $event"
          @ensure-donation-draft="ensureDonationDraft"
          @update-donation-draft="updateDonationDraft"
          @clear-donation-draft="clearDonationDraft"
          @apply-donation-preset="({ donorId, value }) => applyDonationPreset(donorId, value)"
          @open-donation-modal="openDonationModal"
          @toggle-donor-status="toggleDonorStatus"
          @remove-donor="removeDonor"
        />
